    ...
    if terminal(board):
        ...
        if player(board) == X:
            ...
            for action in actions(board):
                if min_value(result(board,action), max_) > max_:
                    max_ = min_value(result(board,action), max_)
                    option = action
            return option
        elif player(board) == O:
            option = (0,0)
            min_ = math.inf
            for action in actions(board):
                if max_value(result(board,action), min_) < min_:
                    min_ = max_value(result(board,action), min_)
                    option = action
            return option

#optimisation of the minimax algorithm with alpha-beta pruning

def max_value(board, min_):
    if terminal(board):
        return utility(board)
    v = -math.inf
    for action in actions(board):
        v = max(v, min_value(result(board, action), min_))
        if v >= min_:
            return v
    return v

def min_value(board, max_):
    if terminal(board):
        return utility(board)
    v = math.inf
    for action in actions(board):
        v = min(v, max_value(result(board, action), max_))
        if v <= max_:
            return v
    return v