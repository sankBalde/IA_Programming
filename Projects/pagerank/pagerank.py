...
def iterate_pagerank(corpus, damping_factor):
    ...
    by more than 0.001 between the current rank values and the new rank values.
    ...
    For each of those incoming pages, let NumLinks(i) be the number of links on page i.
     ...
     that we are on page i at any given time. And since from page i we travel to any of that page’s 
     links with equal probability, we divide PR(i) by the 
    number of links NumLinks(i) to get the probability that we were on page i and chose the link to page p.
    PR(p) = (1 - d) / N + d * (PR(i) / NumLinks(i) + PR(j) / NumLinks(j) + ...
    """
    dictionary = dict()
    for key in corpus:
        dictionary[key] = 1/len(corpus)
